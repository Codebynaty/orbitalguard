...
def divergencias():
    """Barragens onde o SATÉLITE discorda do laudo oficial."""
    out = []
    for b in BARRAGENS_REAIS:
...
@app.get("/ranking-fusion")
def ranking_fusion():
    """Barragens ordenadas pelo Risk Fusion Score dinâmico."""
    res = []
    for b in BARRAGENS_REAIS:
        rf = risk_fusion_score(b, votos_db.get(b["id"]))
